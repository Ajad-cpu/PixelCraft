/*
 * PixelCraft - https://github.com/viliusle/PixelCraft
 * author: Vilius L.
 */
...
class Base_gui_class {
	...
	set_events() {
		...
		document.getElementById('canvas_pixelcraft').addEventListener('contextmenu', function (e) {
			e.preventDefault();
		}, false);
	}
	...
	check_canvas_offset() {
		...
		var canvas_el = document.getElementById('canvas_pixelcraft').getBoundingClientRect();
		this.canvas_offset.x = canvas_el.left - bodyRect.left;
		this.canvas_offset.y = canvas_el.top - bodyRect.top;
	}

	prepare_canvas() {
		var canvas = document.getElementById('canvas_pixelcraft');
		...
		this.render_canvas_background('canvas_pixelcraft');
