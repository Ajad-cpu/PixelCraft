...
class Base_tools_class {
	...
	events() {
		...
		document.addEventListener('touchmove', function (event) {
			_this.dragMove(event);
			if (event.target.id === "canvas_pixelcraft" && !$('.scroll').has($(event.target)).length)
	...
	set_mouse_info(event) {
		...
		if (event.target.id != 'canvas_pixelcraft' && event.target.id != 'main_wrapper') {
			//outside canvas
			this.mouse_valid = false;
		}
		else {
		...
		if (eventType === 'mousedown' || eventType === 'touchstart') {
			if ((event.target.id != 'canvas_pixelcraft' && event.target.id != 'main_wrapper') || (event.which != 1 && eventType !== 'touchstart')) {
				this.mouse_click_valid = false;
			}
			else {
